...
public class Dungeon {
	...
	public static void init() {
		...
		buddy = new Hero();
		buddy.live();
		GamesInProgress.buddyClass.initHero(buddy);

		buddy.setBuddy(hero);
		buddy.charisBuddy(hero);
	}
	...
	public static void switchLevel( final Level level, int pos ) {
		...
		Light lightB = buddy.buff( Light.class );
		buddy.viewDistance = lightB == null ? level.viewDistance : Math.max( Light.DISTANCE, level.viewDistance );

		buddy.curAction = buddy.lastAction = null;
	...
	public static void observe(){
		...
		//new line of buddy code
		observeBuddy(buddy.viewDistance+1);
	}
	...
	public static void observeBuddy(int dist){
		if (level == null) {
			return;
		}

		level.updateFieldOfView(buddy, level.buddyFOV);

		//BUDDY CODE
		int xb = buddy.pos % level.width();
		int yb = buddy.pos / level.width();

		//left, right, top, bottom
		int lb = Math.max( 0, xb - dist );
		int rb = Math.min( xb + dist, level.width() - 1 );
		int tb = Math.max( 0, yb - dist );
		int bb = Math.min( yb + dist, level.height() - 1 );

		int widthb = rb - lb + 1;
		int heightb = bb - tb + 1;

		int posb = lb + tb * level.width();

		for (int j = tb; j <= bb; j++) {
			BArray.or( level.visited, level.buddyFOV, posb, widthb, level.visited );
			posb+=level.width();
		}

		GameScene.updateFog(lb, tb, widthb, heightb);
