...
public class FogOfWar extends Image {
	...
	*/
	...
	public void draw() {
		...
		if (!toUpdate.isEmpty()){
			boolean aux [] = new boolean[Dungeon.level.heroFOV.length];
			for(int i = 0;i<Dungeon.level.heroFOV.length;++i){
				aux[i] = Dungeon.level.heroFOV[i] || Dungeon.level.buddyFOV[i];
			}
			updateTexture(/*Dungeon.level.heroFOV*/aux, Dungeon.level.visited, Dungeon.level.mapped);
			//updateTexture(Dungeon.level.buddyFOV, Dungeon.level.visited, Dungeon.level.mapped);
		}
