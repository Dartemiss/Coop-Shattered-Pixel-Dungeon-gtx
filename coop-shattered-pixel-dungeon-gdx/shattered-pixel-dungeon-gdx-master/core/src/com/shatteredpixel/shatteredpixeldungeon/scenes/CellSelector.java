...
public class CellSelector extends TouchArea<GameAction> {
	...
	public boolean onKeyDown(NoosaInputProcessor.Key<GameAction> key) {
		...
		switch (key.action) {
			...
			case MOVE_LEFT_BUDD:
				...
				isBuddy = true;
				break;
			case MOVE_BOTTOM_LEFT_BUDD:
				xb = -1;
				yb = 1;
				isBuddy = true;
				break;
			case MOVE_BOTTOM_RIGHT_BUDD:
				xb = 1;
				yb = 1;
				isBuddy = true;
				break;
			case MOVE_TOP_LEFT_BUDD:
				xb = -1;
				yb = -1;
				isBuddy = true;
				break;
			case MOVE_TOP_RIGHT_BUDD:
				xb = 1;
				yb = -1;
				isBuddy = true;
				break;
		...
		if (handled) {
			...
			if(!isBuddy) {
				...
				//buddy
				CharSprite.setMoveInterval(Math.max(0.1f, 0.1f + pressedKeySpeedFactor));
				Point pointBuddy = DungeonTilemap.tileToPoint(Dungeon.buddy.pos);
				pointBuddy.x += xb;
				pointBuddy.y += yb;
				pressedKey = key;
				selectBuddy(DungeonTilemap.pointToTile(pointBuddy));
				isBuddy = false;
			}
		}
	...
	public void selectBuddy(int cell){
		if (enabled && listener != null && cell != -1) {

			listener.onSelectBuddy( cell );
			GameScene.ready();
	...
	public interface Listener {
		...
		void onSelectBuddy(Integer cell);
		String prompt();
	}
}
