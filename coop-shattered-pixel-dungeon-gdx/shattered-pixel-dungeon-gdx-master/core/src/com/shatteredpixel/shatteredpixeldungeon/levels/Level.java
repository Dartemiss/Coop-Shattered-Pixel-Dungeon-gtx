...
public abstract class Level implements Bundlable {
		...
		return new Actor() {
			...
			protected boolean act() {
				...
				if (count < nMobs()) {
					...
					if (Dungeon.hero.isAlive() && mob.pos != -1 && (distance(Dungeon.hero.pos, mob.pos) >= 4 || distance(Dungeon.buddy.pos, mob.pos) >= 4)) {
						GameScene.add( mob );
	...
	public int randomRespawnCell() {
		...
		} while ((Dungeon.level == this && (heroFOV[cell] || buddyFOV[cell]))
				|| !passable[cell]
