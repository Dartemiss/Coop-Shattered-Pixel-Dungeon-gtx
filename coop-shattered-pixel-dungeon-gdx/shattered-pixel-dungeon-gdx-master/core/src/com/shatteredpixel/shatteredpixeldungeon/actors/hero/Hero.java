...
public class Hero extends Char {
	...
	//public static final int STARTING_STR = 15000000;
	...
	//private int attackSkill = 100000000;
	//private int defenseSkill = 50000000;
	...
	public boolean act() {
		...
		if(fieldOfView == null) {
			fieldOfView = Dungeon.level.buddyFOV;
		}
		//boolean [] herofieldOfView = Dungeon.level.heroFOV;
		//boolean [] buddyfieldOfView = Dungeon.level.buddyFOV;
		//boolean [] mixedfieldOfView = new boolean[herofieldOfView.length];

		//for(int i=0;i<fieldOfView.length;++i){
		//fieldOfView[i] = herofieldOfView[i] && buddyfieldOfView[i];
		//}
		//fieldOfView = mixedfieldOfView;
		if (!ready) {
	...
	private boolean actMoveBuddy(HeroAction.Move action){
		if (getCloserBuddy( action.dst )) {
			justMoved = true;
			return true;
	...
	private boolean actPickUpBuddy( HeroAction.PickUp action ) {
		int dst = action.dst;
		if (pos == dst) {
	...
	private boolean actOpenChestBuddy( HeroAction.OpenChest action ) {
		int dst = action.dst;
		if (Dungeon.level.adjacent( pos, dst ) || pos == dst) {
	...
	private boolean actUnlockBuddy( HeroAction.Unlock action ) {
		int doorCell = action.dst;
		if (Dungeon.level.adjacent( pos, doorCell )) {
		...
		} else if (getCloserBuddy( doorCell )) {
	...
	private boolean actDescendBuddy( HeroAction.Descend action ) {
		int stairs = action.dst;
		if (pos == stairs && pos == Dungeon.level.exit) {
	...
	private boolean actAttackBuddy( HeroAction.Attack action ) {
		...
		if (enemy.isAlive() && canAttack( enemy ) && !isCharmedBy( enemy )) {
			...
			if (fieldOfView[enemy.pos] && getCloserBuddy( enemy.pos )) {
	...
	private boolean getCloserBuddy( final int target ) {
	...
	public boolean handleBuddy(int cell){
		if (cell == -1) {
			return false;
		}
		Char ch;
		...
		//out.println("fsfsfaf");
		//}
		int dist = Dungeon.level.distance( pos, buddy.pos );
		try {
			PrintWriter out = new PrintWriter("game_info.txt");
			out.println(Integer.toString(dist));
			out.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		//PrintWriter out = new PrintWriter("C:\\Users\\Riqui\\Documents\\TFG\\SerpentAI\\plugins\\SerpentPixelDungeonGameAgentPlugin\\files\\helpers\\game_info.txt");

		if (/*fieldOfView[cell] &&*/ (ch = Actor.findChar( cell )) instanceof Mob) {

			if (ch instanceof NPC) {
				curAction = new HeroAction.Interact( (NPC)ch );
			} else {
				curAction = new HeroAction.Attack( ch );
				boolean aux = actAttackBuddy((HeroAction.Attack)curAction);
			}
			...
			switch (heap.type) {
				case HEAP:
					...
					boolean aux = actPickUpBuddy((HeroAction.PickUp)curAction);
					break;
				case FOR_SALE:
				...
				default:
					...
					boolean aux1 = actOpenChestBuddy((HeroAction.OpenChest)curAction);
			...
			boolean aux = actUnlockBuddy((HeroAction.Unlock)curAction);

		}
		else if (cell == Dungeon.level.exit && Dungeon.depth < 26) {

			curAction = new HeroAction.Descend( cell );
			boolean aux = actDescendBuddy((HeroAction.Descend)curAction);
			...
			lastAction = null;
			boolean aux = actMoveBuddy( (HeroAction.Move)curAction );
			//sprite.move(pos,cell);
			//move(cell);
