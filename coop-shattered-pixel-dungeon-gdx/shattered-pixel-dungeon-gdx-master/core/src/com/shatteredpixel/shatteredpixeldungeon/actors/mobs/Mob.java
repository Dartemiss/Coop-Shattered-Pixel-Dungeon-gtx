...
public abstract class Mob extends Char {
	...
	protected Char chooseEnemy() {
		...
		else if (buff( Amok.class ) != null && (enemy == Dungeon.hero || enemy == Dungeon.buddy))
			newEnemy = true;
			...
			if ( buff(Amok.class) != null) {
				...
				if (enemies.isEmpty()) {
					...
					if (enemies.isEmpty()) {
						...
						if(Dungeon.level.distance(Dungeon.hero.pos,pos)<Dungeon.level.distance(Dungeon.buddy.pos,pos)){
							//try to find the hero third
							if (fieldOfView[Dungeon.hero.pos]) {
								enemies.add(Dungeon.hero);
							}
						}else{
							if (fieldOfView[Dungeon.buddy.pos]) {
								enemies.add(Dungeon.buddy);
							}
						}
			...
			if (enemies.isEmpty()){
				...
				for (Char curr : enemies){
					if (closest == null
							...
							|| Dungeon.level.distance(pos, curr.pos) == Dungeon.level.distance(pos, closest.pos) && (curr == Dungeon.hero || curr == Dungeon.buddy)){
						closest = curr;
					}
				}
				return closest;
	...
	protected boolean moveSprite( int from, int to ) {
		...
		if (sprite.isVisible() && ((Dungeon.level.heroFOV[from] || Dungeon.level.heroFOV[to]) || (Dungeon.level.buddyFOV[from] || Dungeon.level.buddyFOV[to]))) {
			sprite.move( from, to );
			return true;
		} else {
	...
	protected boolean doAttack( Char enemy ) {
		...
		boolean visible = Dungeon.level.heroFOV[pos] || Dungeon.level.buddyFOV[pos] ;
	...
	public boolean surprisedBy( Char enemy ){
		return !enemySeen && (enemy == Dungeon.hero || enemy == Dungeon.buddy);
	}
