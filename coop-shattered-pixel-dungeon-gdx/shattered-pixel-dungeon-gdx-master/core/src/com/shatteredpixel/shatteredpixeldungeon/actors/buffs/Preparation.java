...
public class Preparation extends Buff implements ActionIndicator.Action {
	...
	private CellSelector.Listener attack = new CellSelector.Listener() {
		...
		@Override
		public void onSelectBuddy(Integer cell) {
			if (cell == null) return;
			final Char enemy = Actor.findChar( cell );
			if (enemy == null || Dungeon.buddy.isCharmedBy(enemy) || enemy instanceof NPC){
				GLog.w(Messages.get(Preparation.class, "no_target"));
			} else {

				//just attack them then!
				if (Dungeon.buddy.canAttack(enemy)){
					if (Dungeon.buddy.handle( cell )) {
						Dungeon.buddy.next();
					}
					return;
				}
				...
				boolean[] passable = Dungeon.level.passable.clone();
				PathFinder.buildDistanceMap(Dungeon.buddy.pos, passable, lvl.blinkDistance+1);
				if (PathFinder.distance[cell] == Integer.MAX_VALUE){
					GLog.w(Messages.get(Preparation.class, "out_of_reach"));
					return;
				}
				...
				// but not when actually jumping to a location
				for (Char ch : Actor.chars()){
					if (ch != Dungeon.buddy)  passable[ch.pos] = false;
				}

				PathFinder.Path path = PathFinder.find(Dungeon.buddy.pos, cell, passable);
				int attackPos = path == null ? -1 : path.get(path.size()-2);

				if (attackPos == -1 ||
						Dungeon.level.distance(attackPos, Dungeon.buddy.pos) > lvl.blinkDistance){
					GLog.w(Messages.get(Preparation.class, "out_of_reach"));
					return;
				}

				Dungeon.buddy.pos = attackPos;
				Dungeon.level.press(Dungeon.buddy.pos, Dungeon.buddy);
				//prevents the hero from being interrupted by seeing new enemies
				Dungeon.observe();
				Dungeon.buddy.checkVisibleMobs();

				Dungeon.buddy.sprite.place( Dungeon.buddy.pos );
				Dungeon.buddy.sprite.turnTo( Dungeon.buddy.pos, cell);
				CellEmitter.get( Dungeon.buddy.pos ).burst( Speck.factory( Speck.WOOL ), 6 );
				Sample.INSTANCE.play( Assets.SND_PUFF );

				if (Dungeon.buddy.handleBuddy( cell )) {
					Dungeon.buddy.next();
				}
			}
		}
